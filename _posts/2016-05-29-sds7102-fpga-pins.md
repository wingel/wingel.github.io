...
I used a trick where I configures all unknown GPIO pins as inputs and
write a device driver to detect if the pins changed.  This worked so
...
use the internal 50MHz configuration clock in the Spartan 6 to clock a
counter that toggled the I/O pin every time it wrapped around.  I
built an FPGA image using the Rhea build system and then used my FPGA
device driver to configure the FPGA and load the image.
...
and is toggling at 133MHz.  Hmm, these signals are is bank 2 together
