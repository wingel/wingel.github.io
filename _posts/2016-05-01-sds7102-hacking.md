...
is pretty crappy in some of ways though.  It's fairly noisy so don't
expect to see any details for signals below 1mV at X10 attenuation.
The firmware is a bit buggy and some things just don't work.  Trying
to save images to a USB memory hangs until the USB memory is
unplugged.  Trying to talk to the scope over ethernet crashes the
scope.  The menu system is a bit messy.

Still, it has been very useful.  I have a much fancier, and much
expensive, Hameg scope at the office but for some reason I end up
using the OWON a lot more.  The Hameg has a better analog section, the
noise is noticeably lower than with the OWON, it has a higher sampling
rate, can decode a bunch of different protocols, has a logic analyzer,
and has more math functions than I can shake a soldering iron at and
is just better in so many ways.  It's a solid piece of German
engineering.  But engineers aren't always that good at user interfaces
so it's also a bit clunky to use.
...
that there's not much interesting to see here.  All the interesting
stuff is hidden beneath a shield that I don't would prefer to remove
if I can avoid it.  Someone called "tinhead" on the EEVblog forums has
already [reverse engineered the analog frontend
(AFE)](http://www.eevblog.com/forum/testgear/review-of-owon-sds7102/msg65573/#msg65573)
so hopefully I won't have to open it up.

Components on the back
=======================

I'm more interested in the digital parts and the back of the main
board has a lot more interesting stuff.
...
anything, but after digging a bit more it turns out that it is ought
